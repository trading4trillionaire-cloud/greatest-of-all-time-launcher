...
import com.goat.app.databinding.ActivityCleanStorageUnlockBinding
...
class CleanStorageUnlockActivity : AppCompatActivity() {

    private lateinit var binding: ActivityCleanStorageUnlockBinding

    private var rewardedAd: RewardedAd? = null
    private var isLoadingAd = false
    private var isAdInitialized = false

    // True when the user tapped Open before the ad had finished loading —
    ...
    companion object {
        ...
        const val PREFS_NAME = "goat_storage_unlock_prefs"
        const val KEY_LAST_UNLOCK_TIME = "last_unlock_time"

        const val UNLOCK_WINDOW_MS = 3L * 60 * 1000

        const val EXTRA_SIZE_LABEL = "extra_size_label"
    }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityCleanStorageUnlockBinding.inflate(layoutInflater)
        setContentView(binding.root)

        binding.tvStorageSize.text = intent.getStringExtra(EXTRA_SIZE_LABEL).orEmpty()
            ...
            object : RewardedAdLoadCallback() {
                ...
                override fun onAdFailedToLoad(error: LoadAdError) {
                    ...
                    if (showAdOnceLoaded) {
                        showAdOnceLoaded = false
                        Toast.makeText(
                            this@CleanStorageUnlockActivity,
    ...
    private fun unlockAndProceed() {
        ...
        // NOTE: Which screen opens after the reward is decided later — for now we just
        // close this screen and drop the user back on the launcher.
        goToLauncher()
    }
